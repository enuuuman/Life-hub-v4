  ...
  return (
    <div className="p-5 pb-28 space-y-5 text-slate-800 dark:text-slate-100">
      <section className="space-y-1">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            type="button"
            onClick={handleShare}
            className="min-h-11 shrink-0 rounded-xl bg-indigo-600 px-3 text-white flex items-center gap-2 text-sm font-semibold active:scale-[0.98]"
          >
            <Share2 size={17} /> 共有
          </button>
        </div>
      ...
      <section className="rounded-3xl bg-gradient-to-br from-slate-900 to-indigo-950 p-5 text-white shadow-lg shadow-indigo-950/10">
        <p className="text-xs text-white/60">新居スタート総予算</p>
        <p className="mt-1 text-3xl font-bold tracking-tight">{yen(totals.budget)}</p>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <p className="font-semibold">{yen(totals.spent)}</p>
          </div>
          <div>
            <p className="text-white/55">残り</p>
            <p className="font-semibold">{yen(totals.remaining)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500">{items.filter((item) => item.done).length} / {items.length} 完了</p>
          </div>
          <button
            type="button"
            onClick={() => setShowAdd((value) => !value)}
            className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold flex items-center gap-1.5 dark:border-slate-700 dark:bg-slate-800"
          >
            <Plus size={17} /> 追加
          </button>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {(['all', 'must', 'compromise', 'move', 'reserve'] as const).map((category) => (
            <button
              ...
              className={`min-h-10 shrink-0 rounded-full border px-3 text-xs font-semibold transition-colors ${filter === category ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-200 bg-white text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300'}`}
            >
        ...
        {showAdd && (
          <form onSubmit={addItem} className="rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4 space-y-3 dark:border-indigo-900 dark:bg-indigo-950/30">
            <input
              ...
              placeholder="品名 例：テレビ"
              className="w-full min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-base outline-none focus:border-indigo-400 dark:border-slate-700 dark:bg-slate-900"
            />
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                onChange={(event) => setNewItem((current) => ({ ...current, budget: clampMoney(Number(event.target.value)) }))}
                placeholder="予算"
                className="min-h-11 min-w-0 rounded-xl border border-slate-200 bg-white px-3 text-base outline-none focus:border-indigo-400 dark:border-slate-700 dark:bg-slate-900"
              />
              <select
                value={newItem.category}
                onChange={(event) => setNewItem((current) => ({ ...current, category: event.target.value as BudgetCategory }))}
                className="min-h-11 min-w-0 rounded-xl border border-slate-200 bg-white px-3 text-base outline-none dark:border-slate-700 dark:bg-slate-900"
              >
              ...
              <article key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
                <div className="flex items-center gap-3 p-3.5">
                  <button
                    ...
                    aria-label={`${item.name}を${item.done ? '未購入' : '購入済み'}にする`}
                    className={`h-10 w-10 shrink-0 rounded-xl border flex items-center justify-center ${item.done ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-slate-200 text-slate-300 dark:border-slate-600'}`}
                  >
                    <Check size={20} />
                  </button>
                ...
                {isOpen && (
                  <div className="border-t border-slate-100 p-4 space-y-3 dark:border-slate-700">
                    <div className="grid grid-cols-2 gap-2">
                      <label className="text-xs text-slate-500">予算
                        <input
                          ...
                          step="1000"
                          value={item.budget}
                          onChange={(event) => updateItem(item.id, { budget: clampMoney(Number(event.target.value)) })}
                          className="mt-1 w-full min-h-11 rounded-xl border border-slate-200 bg-slate-50 px-3 text-base text-slate-800 outline-none focus:border-indigo-400 dark:border-slate-600 dark:bg-slate-900 dark:text-white"
                        />
                      </label>
                      <label className="text-xs text-slate-500">実際の金額
                        <input
                          ...
                          value={item.actual}
                          onChange={(event) => {
                            const actual = clampMoney(Number(event.target.value));
                            updateItem(item.id, { actual, done: actual > 0 ? true : item.done });
                          }}
                          className="mt-1 w-full min-h-11 rounded-xl border border-slate-200 bg-slate-50 px-3 text-base text-slate-800 outline-none focus:border-indigo-400 dark:border-slate-600 dark:bg-slate-900 dark:text-white"
                        />
                      </label>
                    ...
                    <label className="block text-xs text-slate-500">支払者
                      <select
                        value={item.payer}
                        onChange={(event) => updateItem(item.id, { payer: event.target.value as Payer })}
                        className="mt-1 w-full min-h-11 rounded-xl border border-slate-200 bg-slate-50 px-3 text-base text-slate-800 outline-none dark:border-slate-600 dark:bg-slate-900 dark:text-white"
                      >
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1">
                      ...
                      {item.id.startsWith('custom-') && (
                        <button
                          ...
                          className="min-h-10 rounded-lg px-2 text-rose-500 flex items-center gap-1 text-xs font-semibold"
                        >
                          <Trash2 size={15} /> 削除
                        </button>
                      )}
