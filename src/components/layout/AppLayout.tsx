...
import { Home, Edit3, Map, TrendingUp, Settings, Download, Upload, Sparkles, Truck } from 'lucide-react';
        ...
        <header className="pt-safe-top bg-white dark:bg-slate-800 shadow-sm z-10">
          <div className="px-5 py-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowSettings(!showSettings)} className="text-slate-400 p-2" aria-label="設定">
                <Settings size={20} />
              </button>
            </div>
          </div>
        ...
        <nav className="absolute bottom-0 w-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border-t border-slate-200 dark:border-slate-700 pb-safe-bottom z-20">
          <div className="flex justify-around items-center h-16">
            {[
              ...
              { id: 'move', icon: Truck, label: '新居' },
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => { setActiveTab(tab.id as TabType); setShowSettings(false); }}
                className={`flex flex-col items-center justify-center min-w-0 flex-1 h-full space-y-1 transition-colors ${activeTab === tab.id && !showSettings ? 'text-indigo-600' : 'text-slate-400'}`}
              >
                <tab.icon size={21} strokeWidth={activeTab === tab.id && !showSettings ? 2.5 : 2} />
